...
def apply_rules_score(feature_df, df):
    ...
    (강화된) 정밀도 우선 버전:
      - (필수조건) lost_count >= 1
      - (필수조건) '부실 대출이 전체 대출 중 최대' (cond_safety)
      - 나머지 점수로 (조기상환 횟수, 비율, 부실금액 배수) 계산
      - threshold=2 (점수 2점 이상이면 (1))
    """
    # '부실 건이 있어야만' + '가장 큰 대출이 부실이어야만' → 필수조건
    must_cond_list = []

    # 1) 부실 건이 1개 이상
    cond_lost = (feature_df['lost_count'] >= 1)
    must_cond_list.append(cond_lost)

    # 2) 부실 대출 == 전체 대출 중 최대 금액
    max_loan_amt_list = []
    for i, row in feature_df.iterrows():
        max_amt = get_user_max_loan_amt(df, row['userId'])
    ...
    must_cond_list.append(cond_safety)

    # 최종 must_cond = (lost_count >=1) & (가장 큰 대출 부실)
    must_cond = np.logical_and.reduce(must_cond_list)

    # 점수는 0부터 시작
    score = np.zeros(len(feature_df))

    # 1) 조기상환 횟수 >=3 → +1
    cond_early_cnt = (feature_df['early_repay_count'] >= 3)
    ...
    cond_early_ratio = (early_ratio >= 0.3)
    score += cond_early_ratio.astype(int)

    # 3) 부실금액 >= 조기상환평균 * 2 → +1
    cond_lost_factor = (
        feature_df['max_lost_loan_amt'] >= feature_df['avg_early_loan_amt'] * 2
    )
    score += cond_lost_factor.astype(int)

    # 임계값
    threshold = 2  # 점수 2 이상이면 (1)
...
def main():
    ...
    print("=== Classification Report: o1-13 ===")
