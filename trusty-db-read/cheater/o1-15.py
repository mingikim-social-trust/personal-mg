...
def apply_rules_score(feature_df, df):
    """
    (강화된) 정밀도 극대화 버전:
      - 필수조건1: lost_count >= 2  (부실 2회 이상)
      - 필수조건2: '부실 대출 중 최대 loanAmount == 전체 대출 중 최대 loanAmount'
      - 점수 항목:
         (a) early_repay_count >= 4 -> +1
         (b) 조기상환 비율 >= 0.4 -> +1
         (c) 부실금액 >= 조기상환평균 * 2.5 -> +1
      - threshold=3
    """
    # ===== (1) 필수조건 구성 =====
    # 부실 기록 최소 2건
    cond_lost_count = (feature_df['lost_count'] >= 2)
    ...
    # 모든 필수조건 AND
    must_cond = (cond_lost_count & cond_safety)
...
def main():
    ...
    print("=== Classification Report: o1-15 ===")
