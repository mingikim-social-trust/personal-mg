...
def apply_rules_score(feature_df, df):
    """
    (정밀도 ↑, 그러나 재현율 0 방지는 기본)
      - 필수조건1: lost_count >= 1 (부실 최소 1회)
      - 필수조건2: '가장 큰 대출이 부실인지'
      - 점수 항목:
         (a) early_repay_count >= 3 -> +1
         (b) 조기상환 비율 >= 0.3  -> +1
         (c) 부실금액 >= 조기상환평균 * 2.0 -> +1
      - threshold=3
    """
    # ========== (1) 필수조건 =============
    # 부실 1회 이상
    cond_lost_count = (feature_df['lost_count'] >= 1)
    ...
    # (c) 부실금액 >= 조기상환평균 * 2.0 -> +1
    cond_lost_factor = (
        feature_df['max_lost_loan_amt'] >= feature_df['avg_early_loan_amt'] * 2.0
    )
    score += cond_lost_factor.astype(int)

    # ========== (3) 임계값 설정 ===========
    threshold = 3  # 3점 이상 + 필수조건 충족 시 (1)
...
def main():
    ...
    print("=== Classification Report: o1-16 ===")
