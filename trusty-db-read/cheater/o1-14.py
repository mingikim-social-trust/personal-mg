...
def apply_rules(feature_df):
    """
    최종 개선된 룰 (가중치 기반):
      각 조건에 엄격한 임계값을 적용하여, 총점이 5점(최대 5점)일 때만 꾼(1)으로 예측합니다.
      
      조건 및 부여 점수:
         1) early_repay_count >= 4              --> +1
         2) (early_repay_count / total_loan_count) >= 0.45   --> +1
         3) lost_count >= 5                      --> +2  (부실 기록이 뚜렷해야 하므로)
         4) max_lost_loan_amt >= (avg_early_loan_amt * 4.0)  --> +1
         
      총점이 5점이면 꾼(1)으로 예측, 그렇지 않으면 Non-꾼(0)으로 예측합니다.
      
      이 방식은 재현율이 다소 낮아질 수 있으나, 정밀도를 크게 높여 false positive를 줄이는 데 초점을 둡니다.
    """
    # 전체 대출 대비 조기상환 비율 계산 (total_loan_count가 0이면 1로 대체하여 나눗셈 에러 방지)
    ...
    score += (feature_df['early_repay_count'] >= 4).astype(int)                    # +1: 조기상환 건수
    score += (early_ratio >= 0.45).astype(int)                                       # +1: 조기상환 비율
    score += (feature_df['lost_count'] >= 5).astype(int) * 2                        # +2: 부실 건수
    score += (feature_df['max_lost_loan_amt'] >= feature_df['avg_early_loan_amt'] * 4.0).astype(int)  # +1: 부실금액 조건
    
    # 총 점수가 5점이면 꾼(1), 그렇지 않으면 Non-꾼(0)
    predict_kkun = np.where(score == 5, 1, 0)
    return predict_kkun
...
def main():
    ...
    print("=== Classification Report: o1-14 ===")
