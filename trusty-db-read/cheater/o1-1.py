...
def apply_rules(feature_df):
    """
    아주 단순한 룰:
      1) early_repay_count >= 3
      2) max_lost_loan_amt >= avg_early_loan_amt * 2  (조기상환 평균금액 대비 2배 이상 큰 대출에서 부실)
      3) lost_count >= 1  (부실기록이 실제로 있어야 함)
    -> 이 모두 만족하면 '꾼(1)', 아니면 '0'
    """
    conditions = (
        (feature_df['early_repay_count'] >= 3) &
