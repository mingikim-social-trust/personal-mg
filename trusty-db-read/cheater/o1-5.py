...
def apply_rules(feature_df):
    """
    정밀도가 너무 떨어지지 않도록 (FP도 줄이고, 1도 적절히 예측) 
    → 과도하게 빡빡했던 조건들을 조금 낮춘 예시
    """
    # 조기상환 비율 = early_repay_count / total_loan_count
    ...
    # --- (1) 완화된 조건 예시 ---
    conditions = (
        (feature_df['early_repay_count'] >= 3) &            # 조기상환 최소 3회
        (early_ratio >= 0.3) &                              # 조기상환 비율 30% 이상
        (feature_df['lost_count'] >= 1) &                   # 부실 기록 최소 1건
        (feature_df['max_lost_loan_amt'] >= feature_df['avg_early_loan_amt'] * 2)  # 부실금액이 조기상환평균의 2배 이상
    )
    
    predict_kkun = np.where(conditions, 1, 0)
    return predict_kkun
...
def main():
    ...
    print("=== Classification Report: o1-5 ===")
