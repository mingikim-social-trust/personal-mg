...
def apply_rules(feature_df):
    """
    개선된 룰:
      - 조건별로 점수를 부여하고, 총 점수가 3점 이상이면 꾼(1)으로 분류합니다.
      
      조건 및 부여 점수:
         1) early_repay_count >= 3          --> +1
         2) (early_repay_count / total_loan_count) >= 0.3   --> +1
         3) lost_count >= 2                  --> +1
         4) max_lost_loan_amt >= (avg_early_loan_amt * 2.5)  --> +1
      
      총점이 3점 이상이면 꾼(1)으로 예측.
    """
    # 전체 대출 대비 조기상환 비율 계산 (total_loan_count가 0이면 1로 대체하여 에러 방지)
    ...
    score += (early_ratio >= 0.3).astype(int)
    score += (feature_df['lost_count'] >= 2).astype(int)
    score += (feature_df['max_lost_loan_amt'] >= feature_df['avg_early_loan_amt'] * 2.5).astype(int)
    
    # 총 점수가 3점 이상이면 꾼(1), 아니면 Non-꾼(0)
    predict_kkun = np.where(score >= 3, 1, 0)
    return predict_kkun
...
def main():
    ...
    print("=== Classification Report: o1-8 ===")
