...
def apply_rules(feature_df):
    """
    개선된 룰: 각 조건에 대해 점수를 부여하고, 총 점수가 일정 임계치 이상이면 꾼(1)으로 분류합니다.
    
    조건 및 부여 점수:
      - 조기상환 건수(early_repay_count) >= 3  --> +1
      - 부실 대출 건수(lost_count) >= 1            --> +1
      - 최대 부실 대출 금액(max_lost_loan_amt) >= (avg_early_loan_amt * 2)  --> +1
      - 조기상환 비율(early_repay_count / total_loan_count) >= 0.15   --> +1
      
    총 점수가 2점 이상이면 꾼(1), 그렇지 않으면 Non-꾼(0)으로 분류합니다.
    """
    # 조건에 따라 점수를 부여
    score = np.zeros(len(feature_df))
    
    # 조건 1: 조기상환 건수가 3건 이상
    score += (feature_df['early_repay_count'] >= 3).astype(int)
    
    # 조건 2: 부실 건수가 1건 이상
    score += (feature_df['lost_count'] >= 1).astype(int)
    
    # 조건 3: 최대 부실 대출 금액 >= 평균 조기상환 대출 금액의 2배 이상
    score += (feature_df['max_lost_loan_amt'] >= feature_df['avg_early_loan_amt'] * 2).astype(int)
    
    # 조건 4: 전체 대출 중 조기상환 비율이 15% 이상 (단, total_loan_count가 0인 경우는 제외)
    ratio = feature_df['early_repay_count'] / feature_df['total_loan_count'].replace(0, np.nan)
    score += (ratio >= 0.15).fillna(0).astype(int)
    
    # 총 점수가 2점 이상이면 꾼으로 판별
    predict_kkun = np.where(score >= 2, 1, 0)
    return predict_kkun
...
def main():
    ...
    print("=== Classification Report: o1-4 ===")
