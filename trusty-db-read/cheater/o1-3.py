...
def apply_rules(feature_df):
    """
    개선된 룰:
      1) early_repay_count >= 4
      2) lost_count >= 2
      3) max_lost_loan_amt >= (avg_early_loan_amt * 3)
      4) (early_repay_count / total_loan_count) >= 0.2  # 전체 대출 중 20% 이상이 조기 상환
    """
    conditions = (
        (feature_df['early_repay_count'] >= 4) &
        (feature_df['lost_count'] >= 2) &
        (feature_df['max_lost_loan_amt'] >= feature_df['avg_early_loan_amt'] * 3) &
        ((feature_df['early_repay_count'] / feature_df['total_loan_count']) >= 0.2)
    )
...
def main():
    ...
    print("=== Classification Report: o1-3 ===")
