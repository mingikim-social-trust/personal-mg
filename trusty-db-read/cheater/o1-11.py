...
def apply_rules_score(feature_df, df):
    ...
    정밀도 강화 버전:
     ...
     2) 조기상환 횟수 >= 4 → +1점 (상향)
     3) 조기상환 비율 >= 0.4 → +1점 (상향)
     4) 부실금액 >= 조기상환평균 * 2.5 → +1점 (상향)
     5) '부실 대출이 전체 대출 중 최대'면 +2점 (다시 +2 가중치)
     → 최종 점수가 4 이상이면 (1)
    """
    # 필수조건: 부실 기록 있어야만 (1) 후보
    ...
    # --- (1) 조기상환 횟수 >= 4 -> +1 ---
    cond_early_cnt = (feature_df['early_repay_count'] >= 4)
    ...
    cond_early_ratio = (early_ratio >= 0.4)
    score += cond_early_ratio.astype(int)

    # --- (3) 부실금액 >= 조기상환평균의 2.5배 -> +1 ---
    cond_lost_factor = (
        feature_df['max_lost_loan_amt'] >= feature_df['avg_early_loan_amt'] * 2.5
    )
    score += cond_lost_factor.astype(int)

    # --- (4) 안전장치: '부실 대출 == 전체 대출 중 최대 loanAmount' -> +2점 ---
    max_loan_amt_list = []
    ...
    score += (cond_safety.astype(int) * 2)  # +2
...
def main():
    ...
    print("=== Classification Report: o1-11 ===")
