...
def apply_rules_score(feature_df, df):
    """
    1) (필수조건) lost_count >= 1     (부실이 아예 없으면 '꾼'으로 보지 않음)
    2) 여러 지표에 대해 점수를 부여. (조기상환 횟수, 조기상환 비율, 부실금액 배수, etc.)
    3) '가장 큰 대출이 부실인가?' 같은 추가 안전장치 -> 점수를 크게 부여
    4) 최종 점수가 threshold 이상이면 '꾼(1)'으로 분류
    """
    must_cond = (feature_df['lost_count'] >= 1)

    # 우선 모든 행에 대해 0점으로 시작
    score = np.zeros(len(feature_df))

    # --- (예시1) 조기상환 횟수 >= 2 --> +1점 ---
    cond_early_cnt = (feature_df['early_repay_count'] >= 2)
    score += cond_early_cnt.astype(int)

    # --- (예시2) 조기상환 비율 >= 0.2 --> +1점 ---
    early_ratio = feature_df['early_repay_count'] / np.where(
        feature_df['total_loan_count'] > 0,
        feature_df['total_loan_count'],
        1
    )
    cond_early_ratio = (early_ratio >= 0.2)
    score += cond_early_ratio.astype(int)

    # --- (예시3) 부실금액 >= (조기상환평균의 1.8배) --> +1점 ---
    cond_lost_factor = (
        feature_df['max_lost_loan_amt'] >= feature_df['avg_early_loan_amt'] * 1.8
    )
    score += cond_lost_factor.astype(int)

    # --- (예시4) 안전장치: 부실 대출이 전체 대출 중 '최대 loanAmount'였다면 +2점 ---
    #     "정말 크게 한 번 부실냈을 가능성"을 가정해서 가중치 +2
    max_loan_amt_list = []
    for i, row in feature_df.iterrows():
        user_id = row['userId']
        max_amt = get_user_max_loan_amt(df, user_id)
        max_loan_amt_list.append(max_amt)

    max_loan_amt_series = pd.Series(max_loan_amt_list, index=feature_df.index)
    cond_safety = (feature_df['max_lost_loan_amt'] == max_loan_amt_series)
    score += (cond_safety.astype(int) * 2)  # 가중치 +2

    # --- 임계값(Threshold) 설정 ---
    #     예: 점수가 '3점 이상'이면 꾼(1)로 분류
    threshold = 3

    # 최종 예측: (필수조건) & (점수 >= threshold)
    predict_kkun = np.where((must_cond) & (score >= threshold), 1, 0)
    return predict_kkun


def get_user_max_loan_amt(df, userId):
    """유저 전체 대출 건 중 가장 큰 loanAmount를 반환"""
    user_rows = df[df['userId'] == userId]
    return user_rows['loanAmount'].max()
...
def main():
    ...
    # 3. 새로운 룰(점수+안전장치) 기반 분류
    feature_df['predict_kkun'] = apply_rules_score(feature_df, df)
    ...
    print("=== Classification Report: o1-7 ===")
