...
def apply_rules(feature_df):
    """
    정밀도(precision) 향상을 위해 기존 룰을 더 타이트하게 수정한 예시:
      - 조기상환 횟수 >= 4
      - lost_count >= 1  (여기선 그대로 두었지만 2로 해도 됨)
      - max_lost_loan_amt >= avg_early_loan_amt * 3 (2에서 3으로 상향)
      - 조기상환 비율 >= 0.5  (ex: 조기상환한 건이 전체 대출의 절반 이상)
    """
    # 조기상환 비율 계산(분모가 0인 경우 대비)
    early_ratio = feature_df['early_repay_count'] / np.where(feature_df['total_loan_count'] > 0,
                                                            feature_df['total_loan_count'],
                                                            1)
    # 조건
    conditions = (
        (feature_df['early_repay_count'] >= 4) &
        (feature_df['lost_count'] >= 1) &
        (feature_df['max_lost_loan_amt'] >= feature_df['avg_early_loan_amt'] * 3) &
        (early_ratio >= 0.5)
    )
