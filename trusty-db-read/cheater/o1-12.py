...
def apply_rules(feature_df):
    ...
    개선된 룰 (가중치 기반):
      ...
      조건 및 부여 점수:
         ...
         2) (early_repay_count / total_loan_count) >= 0.4   --> +1
         3) lost_count >= 4                      --> +2  
         4) max_lost_loan_amt >= (avg_early_loan_amt * 3.5)  --> +1
    ...
    score += (feature_df['early_repay_count'] >= 4).astype(int)                    # 조건 1: +1
    score += (early_ratio >= 0.4).astype(int)                                       # 조건 2: +1
    score += (feature_df['lost_count'] >= 4).astype(int) * 2                        # 조건 3: +2
    score += (feature_df['max_lost_loan_amt'] >= feature_df['avg_early_loan_amt'] * 3.5).astype(int)  # 조건 4: +1
...
def main():
    ...
    print("=== Classification Report: o1-12 ===")
