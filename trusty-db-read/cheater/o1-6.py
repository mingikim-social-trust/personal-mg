...
def apply_rules(feature_df):
    """
    개선된 룰:
      - 조건별로 점수를 부여하고, 총 점수가 임계치 이상이면 꾼(1)으로 분류합니다.
      
      조건 및 부여 점수:
         1) 조기상환 건수(early_repay_count) >= 2  --> +1
         2) 조기상환 비율(early_repay_count / total_loan_count) >= 0.2  --> +1
         3) 부실 대출 건수(lost_count) >= 1           --> +1
         4) 최대 부실 대출 금액(max_lost_loan_amt) >= (avg_early_loan_amt * 2)  --> +1
         
      총점이 2점 이상이면 꾼으로 예측.
      (임계치는 데이터 분포에 따라 조정 가능)
    """
    # 조기상환 비율 계산 (total_loan_count가 0인 경우 1로 대체하여 나눗셈 에러 방지)
...
def main():
    ...
    print("=== Classification Report: o1-6 ===")
